...
#test:
is_deeply($allQAs_ref, $EXPECTED_ALLQAS_REF);
...
#test sorted arrays:
is_deeply($sortedExamFileLines_ref, \@SORTED_EXPECTED_EXAMFILELINES);

#test lines that shouldn't be in random order:
for my $line (0..18,24..25,31..36,42..44){
    is(@{$examFileLines_ref}[$line], @EXPECTED_EXAMFILELINES[$line]);
}
